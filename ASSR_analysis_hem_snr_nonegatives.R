# ----------------------------------------------------------------------------------------- #
# name              ASSR_analysis_hem_snr_nonegatives.R                                     #
# created by        Shauni Van Herck                                                        #
# description       used to analyse ASSR data for hemispheres (snr without negative values) #
...
d_4_cook         = d_4[ -c(168, 172, 468, 424, 10, 47, 534, 261, 300, 361), ]   #remove data points based on Cook's distance
#[ -c(168, 172, 467, 437, 468, 424, 10, 627, 47, 534, 518, 48, 163, 51, 501, 261, 423, 300, 101, 361, 214, 264, 547, 3), ]
...
tiff("snr_4Hz_hem.tiff",width=1600,height=800)                 #save figure in wd
...
# 4 Hz with data points based on Cook's distance removed (10 points removed)
...
## 20 Hz
leveneTest(snr ~ prepost*stimtype*group, data = d_20)
...
# 20 Hz
options(contrasts=c("contr.sum", "contr.poly"))
fit20             = lmer(snr  ~ group*stimtype*prepost*hem + (1|subject), data=d_20)
