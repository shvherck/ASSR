...
d$GG_accuracy  = GG$Accuracy[match(unlist(d$subject), GG$iCode)]
#use GG dataframe to find correct GG factors for each i-code

# subset data
d_all          = subset(d, Channel%in%c('All'))
d_all          = subset(d, group%in%c('GG_EE', 'GG_NE'))
colnames(d_all)= c("subject", "group", "cond", "stimtype", "freq", "prepost", "channel", "snr", "hours", "levels", "progress", "accuracy")
head(d_all)

d_all_melt           = melt(d_all, id.var=c("subject", "group", "cond", "stimtype", "prepost", "freq", "hours", "levels", "progress", "accuracy"), measure.var=c("snr"))
d_all_wide           = cast(subject + group + cond + stimtype + freq + hours + levels + progress + accuracy ~ prepost , data=d_all_melt, mean)
head(d_all_wide)
d_all_wide$diff      = d_all_wide$post - d_all_wide$pre
colnames(d_all_wide) = c("subject", "group", "cond", "stimtype", "freq", "hours", "levels", "progress", "accuracy", "post", "pre", "diff") 
...
cor.test(corr4$diff, corr4$levels, method="pearson")
cor.test(corr4$diff, corr4$progress, method="pearson")
...
cor.test(corr20$diff, corr20$levels, method="pearson")
cor.test(corr20$diff, corr20$progress, method="pearson")
